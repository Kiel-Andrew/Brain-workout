...
export default function WorkoutLobby({ leaderboard, batches, timerSeconds }: {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("workout_difficulty") as Difficulty;
  ...
  // Save difficulty
  useEffect(() => {
    localStorage.setItem("workout_difficulty", difficulty);
  }, [difficulty]);
  ...
  useEffect(() => {
    ...
    if (countdown === 3 || countdown === 2 || countdown === 1) {
      const timer = setTimeout(() => {
        setCountdown((c) => (c as number) - 1 === 0 ? "GO!" : (c as number) - 1);
      }, 1000);
  ...
  function startGame() {
    ...
    setCountdown(3);
    setGameState("lobby"); // Keep in lobby view but overlay will show
  }
  ...
  function pressKey(key: string) {
    ...
    if (input === "0") {
      ...
      nextInput = key; // replace "0" with new digit
    } else {
      if (input.length < 6) nextInput = input + key;
    }

    setInput(nextInput);

    // Auto-submit logic
    const target = questions[current].answer.toString();
    if (nextInput.length === target.length) {
      submitAnswer(nextInput);
    }
  }

  function submitAnswer(overrideInput?: string) {
    const finalInput = overrideInput ?? input;
    if (!finalInput || feedback) return;
    const ans = parseInt(finalInput);
    ...
    setTimeout(() => {
      ...
      // If wrong, we just clear input and stay on 'current'
    }, 600);
  }
  ...
  const renderPlaying = () => {
    ...
    const ms = Math.floor((totalLiveTime % 1) * 100).toString().padStart(2, "0");
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <div style={{ position: "relative" }}>
              <div style={{
                ...
                <Clock size={16} />{mins}:{secs}.<span style={{ fontSize: 16, opacity: 0.8, marginLeft: 1 }}>{ms}</span>
              </div>
                    ...
                    return (
                      <button key={key} id={`key-${key}`} onClick={() => pressKey(key)} disabled={!!feedback}
                        style={{
                          ...
                          gridColumn: isZero && ri === 3 ? "span 2" : "auto",
          ...
          {countdown !== null && (
            <div style={{
              ...
              <div key={countdown} style={{
                fontSize: 140, fontWeight: 900, color: "white", // Solid white
